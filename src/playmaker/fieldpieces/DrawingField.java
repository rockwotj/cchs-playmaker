...
public class DrawingField extends JComponent
{
	...
	{
		Player playerToAdd = new Player(x, y, Main.numYards, this.width, this.height,
	...
	protected void paintComponent(Graphics g)
	{
		...
		{
			g2.drawLine(0, this.height / Main.numYards * i, this.width / 100, this.height
					/ Main.numYards * i);
			g2.drawLine(this.width, this.height / Main.numYards * i, this.width - this.width / 100,
					this.height / Main.numYards * i);
			g2.drawLine(this.width / 3, this.height / Main.numYards * i, this.width / 3
					+ this.width / 100, this.height / Main.numYards * i);
			g2.drawLine(2 * this.width / 3, this.height / Main.numYards * i, 2 * this.width / 3
					+ this.width / 100, this.height / Main.numYards * i);
			if (i % 5 == 0)
			{
				if ((Main.numYards == 25) && (i == 15))
				{
					...
					g2.drawRect(5 * this.width / 6 + 3, this.height / Main.numYards * i - 22, 24,
							14);
					drawRightOne(g2, 5 * this.width / 6, this.height / Main.numYards * i + 5, 30,
							20);
					...
					drawLeftOne(g2, this.width / 6 - 30, this.height / Main.numYards * i - 25, 30,
							20);
				}
				g2.drawLine(0, this.height / Main.numYards * i, this.width, this.height
						/ Main.numYards * i);
				g2.drawLine(this.width / 3 + this.width / 100, this.height / Main.numYards * i
						- this.height / (3 * Main.numYards), this.width / 3 + this.width / 100,
						this.height / Main.numYards * i + this.height / (3 * Main.numYards));
				g2.drawLine(2 * this.width / 3, this.height / Main.numYards * i - this.height
						/ (3 * Main.numYards), 2 * this.width / 3, this.height / Main.numYards * i
						+ this.height / (3 * Main.numYards));
			}
		}
	...
	public void addTextBox(int x, int y, String string)
	{
		if (string.length() > 0)
		{
			TextBox text = new TextBox(x, y, (Graphics2D) getGraphics(), string, this.width,
					this.height);
	...
	public void exportAsGIF()
	{
		...
		if (show)
		{
			JOptionPane.showMessageDialog(this, Main.fileName + " exported as picture!",
					"Cowboy Play Maker", 1);
		}
	}
