...

{
...

	{
		...
		FileNameExtensionFilter filter = new FileNameExtensionFilter("Playmaker Files",
				new String[] { "roc" });
	...
	private static void save(ArrayList<FieldPieces> pieces, PrintWriter out, File outFile,
	...
	private static ArrayList<FieldPieces> loadFile(File inFile, Graphics2D g2,
			DrawingField drawingField, boolean showMessage) throws FileNotFoundException

	{
		...
		if (showMessage)
		{
			JOptionPane.showMessageDialog(drawingField, Main.fileName + " loaded!",
					"Cowboy Play Maker", 1);
		}
		in.close();
		return piecesInFile;
	...
	private static FieldPieces playerFromString(char[] line, int i, StringBuilder builder,
			int width, int height)
	...
	private static FieldPieces zoneFromString(char[] line, int i, StringBuilder builder,
	...
	private static FieldPieces textBoxFromString(char[] line, int i, StringBuilder builder,
	...
	private static FieldPieces solidLineFromString(char[] line, int i, StringBuilder builder,
			int width, int height)
	...
	private static FieldPieces motionLineFromString(char[] line, int i, StringBuilder builder,
			int width, int height)
	...
	private static FieldPieces dashedLineFromString(char[] line, int i, StringBuilder builder,
			int width, int height)
	...
	private static FieldPieces curvedLineFromString(char[] line, int i, StringBuilder builder,
			int width, int height)

	{
		...
		double y2 = Double.parseDouble(builder.toString());
		clearStringBuilder(builder);
		return new CurvedLine((int) x1, (int) y1, (int) xCtrl, (int) yCtrl, (int) x2, (int) y2,
				width, height);
	}

	private static FieldPieces blockheadFromString(char[] line, int i, StringBuilder builder,
			int width, int height)
	...
	private static FieldPieces arrowheadFromString(char[] line, int i, StringBuilder builder,
			int width, int height)
...

	{
		...
		JFileChooser outFileChooser = new JFileChooser(path);
		FileNameExtensionFilter filter = new FileNameExtensionFilter(fileType.toUpperCase()
				+ " Image Files", new String[] { fileType });
		outFileChooser.setFileFilter(filter);
		outFileChooser.setAcceptAllFileFilterUsed(false);
		File outFile;
		if (outFileChooser.showSaveDialog(null) == 0)
